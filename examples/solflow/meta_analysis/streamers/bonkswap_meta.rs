...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let program_id = "LanMV9sAd7wArD4vJFi2qDdfnVhFxYSUg6eADduJ3uj".to_string();
    let program_name = "BonkSwap".to_string();
    ...
    let output_dir = PathBuf::from("data/meta_analysis/bonkswap/raw");
    tokio::fs::create_dir_all(&output_dir).await?;
    let output_path = output_dir.join(format!("{}_session.jsonl", timestamp));

    log::info!("🔬 BonkSwap Meta-Analysis Capture");
