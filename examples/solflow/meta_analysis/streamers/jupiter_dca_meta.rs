...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let program_name = "JupiterDCA".to_string();
    ...
    let output_dir = PathBuf::from("data/meta_analysis/jupiter_dca/raw");
    tokio::fs::create_dir_all(&output_dir).await?;
    let output_path = output_dir.join(format!("{}_session.jsonl", timestamp));

    log::info!("🔬 Jupiter DCA Meta-Analysis Capture");
